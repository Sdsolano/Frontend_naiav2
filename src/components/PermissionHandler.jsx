...
const PermissionHandler = ({ onAllPermissionsGranted }) => {
  ...
  // Este efecto se ejecuta cuando cambian los permisos para cerrar el modal automáticamente
  useEffect(() => {
  ...
  }, [permissions, onAllPermissionsGranted]);
  ...
  const checkPermissions = async () => {
    try {
      ...
      // Configurar oyentes para cambios en los permisos
      camera.addEventListener('change', checkPermissions);
      microphone.addEventListener('change', checkPermissions);
      geolocation.addEventListener('change', checkPermissions);
      
      return () => {
        // Limpiar oyentes
        camera.removeEventListener('change', checkPermissions);
        microphone.removeEventListener('change', checkPermissions);
        geolocation.removeEventListener('change', checkPermissions);
      };
    } catch (error) {
      ...
      <div className="bg-white p-6 rounded-lg max-w-md w-full">
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div className="text-center">
            <Camera className={`mx-auto h-8 w-8 ${permissions.camera === 'granted' ? 'text-green-800' : 'text-red-800'}`} />
            <p className="mt-2 font-semibold">Camera</p>
          </div>
          <div className="text-center">
            <Mic className={`mx-auto h-8 w-8 ${permissions.microphone === 'granted' ? 'text-green-800' : 'text-red-800'}`} />
            <p className="mt-2 font-semibold">Microphone</p>
          </div>
          <div className="text-center">
            <MapPin className={`mx-auto h-8 w-8 ${permissions.location === 'granted' ? 'text-green-800' : 'text-red-800'}`} />
        ...
        <div className="flex justify-end space-x-2 mt-4">
          ...
          {!isAnyPermissionDenied && (
            <button onClick={requestPermissions} className="px-4 py-2 bg-gray-500 text-white rounded">
              Request Permissions
            </button>
          )}
          {isAnyPermissionDenied && (
