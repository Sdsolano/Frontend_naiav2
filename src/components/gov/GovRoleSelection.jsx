...
import { getGovConfig } from '../../utils/roleUtils';

// Rol único para Gobernación - manteniendo estructura original
const roles = [
  {
    id: "ciudadano",
    icon: Users,
    title: "Asistente de Atención al Ciudadano",
    description: "Tu guía especializada para trámites y servicios gubernamentales del departamento del Atlántico.",
    color: "bg-red-600",
    ...
    features: [
      ...
      "Orientación para procesos administrativos del departamento",
...
const GovRoleSelection = () => {
  ...
  const handleConfirmRole = () => {
    if (selectedRole && selectedRole.available) {
      // Configurar contexto de gobierno
      const govConfig = getGovConfig();
      localStorage.setItem('naia_selected_role', govConfig.roleKey);
      localStorage.setItem('naia_gov_context', 'true');
      localStorage.setItem('naia_user_id', govConfig.userId.toString());
      
      console.log(`🏛️ Contexto gobierno activado: ${selectedRole.title} (${selectedRole.id})`);
      
      // Emitir evento para notificar cambio de rol
      window.dispatchEvent(new CustomEvent('role-changed', { 
        detail: { roleId: selectedRole.id, roleName: selectedRole.title, isGov: true }
      }));
      
      navigate('/gov/naia/interface');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-orange-50 to-white">
      ...
      {/* Footer - ESTRUCTURA ORIGINAL adaptada para Gobierno */}
...
export default GovRoleSelection;