...
import { BookOpen, ArrowRight, X, ChevronLeft, Info } from "lucide-react";
import Thinking_naia from "../../assets/NAIA_greeting.png";
import Companion_AF from "../../assets/Companion_AF.png";
import { getToeflConfig } from '../../utils/roleUtils';

// Rol único para TOEFL - manteniendo estructura original
const roles = [
  {
    id: "toefl-tutor",
    icon: BookOpen,
    title: "Tutora TOEFL Especializada",
    description: "Tu guía personalizada para dominar el examen TOEFL con estrategias avanzadas y práctica intensiva.",
    color: "bg-blue-600",
    ...
    image: Companion_AF,
    features: [
      "Práctica intensiva en las 4 secciones del TOEFL (Reading, Listening, Speaking, Writing)",
      "Estrategias específicas para maximizar tu puntuación",
      "Evaluación en tiempo real de tu progreso y nivel actual",
      "Simulacros de examen con condiciones reales",
      "Retroalimentación personalizada para cada respuesta",
      "Tips y técnicas de expertos para el día del examen"
    ],
    developmentStatus: "Disponible ahora - Especializada en TOEFL iBT"
  }
];

const ToeflRoleSelection = () => {
  const [activeIndex, setActiveIndex] = useState(0); // Siempre 0 ya que hay un solo rol
  ...
  const handleConfirmRole = () => {
    if (selectedRole && selectedRole.available) {
      // Configurar contexto de TOEFL
      const toeflConfig = getToeflConfig();
      localStorage.setItem('naia_selected_role', toeflConfig.roleKey);
      localStorage.setItem('naia_toefl_context', 'true');
      localStorage.setItem('naia_user_id', toeflConfig.userId.toString());

      console.log(`📚 Contexto TOEFL activado: ${selectedRole.title} (${selectedRole.id})`);

      // Emitir evento para notificar cambio de rol
      window.dispatchEvent(new CustomEvent('role-changed', {
        detail: { roleId: selectedRole.id, roleName: selectedRole.title, isToefl: true }
      }));

      navigate('/toefl/naia/interface');
    }
  };

  // Role Details Panel Component - ESTRUCTURA ORIGINAL EXACTA
        ...
        >
          ...
          <div className="overflow-y-auto max-h-[calc(82vh-4rem)] md:max-h-[calc(88vh-4rem)]">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-0">
                  ...
                  <div className="absolute top-4 left-4 sm:top-6 sm:left-6 flex items-center gap-2 bg-white/90 backdrop-blur-sm px-3 py-2 sm:px-4 sm:py-3 rounded-xl shadow-lg">
                    ...
                    <div>
                      ...
                      <div className={`text-xs ${selectedRole.available ? 'text-green-600' : 'text-gray-500'} font-medium`}>
                        {selectedRole.available ? '✓ Disponible' : '⏳ Próximamente'}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* Right column - Contenido adaptativo */}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-white">
      ...
      <header className="relative text-blue-600">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 sm:h-20">
            ...
            <div className="text-center">
              <h1 className="text-lg sm:text-xl font-bold">NAIA TOEFL</h1>
              <p className="text-blue-500 text-xs sm:text-sm">Preparación TOEFL</p>
            </div>
      ...
      <main className="flex-1 overflow-x-hidden">
        <div className="max-w-6xl mx-auto px-2 sm:px-4 w-full">
          {/* Role Carousel/Showcase - ESTRUCTURA ORIGINAL EXACTA */}
          <div className="relative mb-4 sm:mb-8 w-full overflow-hidden">
            {/* 3D Carousel Layout - ESTÁTICO (un solo rol centrado) */}
            <div className="flex justify-center items-center h-[430px] sm:h-[550px] md:h-[700px] overflow-hidden py-4 max-w-full w-full mx-auto px-1">
              {roles.map((role, index) => {
                // Para un solo rol, siempre está activo y centrado
                const isActive = true;
                const zIndex = 30;
                const opacity = 1;
                const scale = 1;
                ...
                return (
                  ...
                  >
                    {/* Card - DIMENSIONES AJUSTADAS MÁS RECTANGULARES */}
                    <div
          ...
          {/* Role Description Section - ESTRUCTURA ORIGINAL EXACTA */}
            ...
            <div className="flex items-start gap-3 sm:gap-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3">
                  ...
                  >
                    <span>{roles[activeIndex].available ? 'Comenzar ahora' : 'Próximamente'}</span>
          ...
          {/* Dots Navigation - NO NECESARIO para un solo rol, pero mantenemos consistencia visual */}
          <div className="flex justify-center mt-6 sm:mt-8">
            <div className="flex gap-2">
              <div className="w-4 sm:w-6 h-2 bg-blue-600 rounded-full" aria-label="Rol actual" />
            </div>
          </div>
        </div>
      </main>

      {/* Footer - ESTRUCTURA ORIGINAL adaptada para TOEFL */}
      <footer className="py-4 sm:py-6 border-t border-gray-200 bg-white/70 backdrop-blur-sm mt-4">
        <div className="max-w-6xl mx-auto px-3 sm:px-6 flex flex-col sm:flex-row justify-between items-center">
          <div className="flex items-center mb-3 sm:mb-0">
            ...
            />
            <div>
              <h2 className="text-base sm:text-lg font-bold bg-gradient-to-r from-blue-600 to-indigo-500 bg-clip-text text-transparent">NAIA TOEFL</h2>
              <p className="text-[10px] sm:text-xs text-gray-500">Preparación TOEFL © 2025</p>
            </div>
          </div>
          <div className="flex items-center space-x-4 sm:space-x-6 text-gray-600">
            <Link to="/toefl" className="text-xs sm:text-sm hover:text-blue-600 transition-colors flex items-center gap-1">
...
export default ToeflRoleSelection;